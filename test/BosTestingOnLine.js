...
describe("Bos Testing", function () {
    ...
    it('token mint', async function () {
        ...
        let _routerContract = await ethers.getContractFactory("Router");
        let routerContract = await _routerContract.deploy();
        await routerContract.deployed();

        await cusdt.mint(routerContract.address,amount);
        console.log("mint init ok");

        console.log(await bosToken.balanceOf(deployer.address))

        await bosToken.connect(deployer).transfer(routerContract.address,"10000000000000000000000000");

        console.log("mint init ok");
    
        await routerContract.addLiquidity(router,bosToken.address,cusdt.address);
    });
